...
class GameWindow < Gosu::Window
  ...
  def restart  #### When you press Z, this method gets called
    ...
    for i in 0..19  ### Repeat 40 times
      @balls << Ball.new(self, rand_range(@univ_left + ball_radius, @univ_right - ball_radius), 
			 ...
			 rand(360), rand(8.0), ball_radius, 0xFF_FF0000, false) 
    end
    for i in 0..79  ### Repeat 40 times
      @balls << Ball.new(self, rand_range(@univ_left + ball_radius, @univ_right - ball_radius), 
			 rand_range(@univ_bottom + ball_radius, @univ_top - ball_radius),
			 rand(360), rand(8.0), ball_radius, 0xFF_0000FF, false) 
    end
    @balls << Ball.new(self, rand_range(@univ_left + ball_radius, @univ_right - ball_radius), 
		       rand_range(@univ_bottom + ball_radius, @univ_top - ball_radius),
		       rand(360), rand(8.0), ball_radius*1.4 , 0xFF_FFFFFF, true) 
  end
  ...
  def draw
    ...
    @font.draw("Press Z to reset",         (@univ_left+@univ_right)/2-100, @univ_bottom-20, 2)
  end
